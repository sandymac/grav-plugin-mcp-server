...
1. [](#improved)
    * Hardening from an external review: the OAuth store is never replaced by an empty file when encoding fails; unknown authorization codes and refresh tokens no longer rewrite the store; client registration caps redirect URIs at 10 of 512 bytes each; the consent page shows the full redirect URI, not just its host; the consent form's signature fails closed on invalid UTF-8; tool failures return a fixed message and log the exception instead of echoing it (a wrong-typed argument used to reveal the server path); MCP responses carry `Cache-Control: no-store`; `get_packages` rejects a type other than `plugins` or `themes`; uploads refuse more than 20 files or a file over 64 MiB before decoding.
...
1. [](#improved)
    * MCP responses now return through Grav's request pipeline, the way the api plugin's REST responses do, instead of being printed and exited. `onShutdown` fires after a tool call exactly as after a REST call, so a plugin that queues email or webhooks there now runs for MCP too, and Grav's output compression and connection handling apply. Tool calls also get the api plugin's own request setup — the page index built lazily on first use, the object cache kept warm when the site's is off — so a tool call and the REST call behind it now run under the same conditions. The OAuth browser flows are unchanged.
    * Triaged against api plugin 1.0.30, 1.0.31 and 1.0.32, which span the Grav 2.0/2.1 boundary. **The api floor rises to 1.0.30** — the release with the corrected gate above, and the last one a Grav 2.0 site can install, since 1.0.31 and 1.0.32 require Grav 2.1.0. The Grav floor follows to 2.0.24, which api 1.0.30 itself requires. 1.0.30 also makes an unscoped key's account the active Grav user for the duration of a call, so plugins and plugin-published tools see who is calling instead of a guest (a key with a scope list still looks like a guest to them). 1.0.31 accepts licence keys from stores other than Grav Premium; 1.0.32 fixes the Grav-upgrade override so it also lifts a pending-plugin-updates block. No other endpoint changes.
