...
check(count($descriptors) === 65, 'tools/list returns all 65 tools, got ' . count($descriptors));
check(
    array_diff(
        [
...
check(count($scoped->list()) === 65, 'an unscoped key sees everything');
...
check(
    $access['visible'] + $access['hidden'] === 65
    && $access['core_tools'] === 65 && $access['plugin_tools'] === 0 // no Grav here, so no manifest
